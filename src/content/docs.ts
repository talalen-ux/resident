...
export const METHOD = [
  ...
  },
  {
    ...
    body: [
      ...
      "Each interval runs the same rules in the same order. Close anything that has stopped earning; collect fees that are worth collecting; re-centre anything that has drifted off the price; move money off a pool that still works but has been overtaken by a better one; put whatever is idle to work in the best pool that qualifies; and only then consider moving money to another chain, which is the one decision that cannot be undone within the interval.",
      "That fourth rule is the one most desks do not have, and its absence does not look like anything. Every position is above its floor, nothing has stopped earning, and the money is sitting in the pool that was the best one when it was deployed rather than the one that is best now. A pool paying many times what a position currently earns is a reason to move, and the move has to clear three times what the round trip costs before it is taken.",
